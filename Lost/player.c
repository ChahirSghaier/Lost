...
{
nc=1;
if(destrect.y>=box.y+box.h-16)
{
ground=1;
yvel=0;
}else if(destrect.y+destrect.h<=box.y+16)
{
box.x++;
yvel=30;
}
if(box.x+box.w>=destrect.x-8 && box.y+box.h>=destrect.y+9 && box.x+box.w<=destrect.x+30)
{
xvel=0;
box.x--;
}else if(box.x<=destrect.x+destrect.w && box.y+box.h>=destrect.y+9)
{
xvel=0;
box.x++;
}
