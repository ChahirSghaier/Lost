...
bool run=true;
const int NUMMENU=2;
const char* array[NUMMENU]={"Jouer","Exit"};
...
if(in.eof())
{
std::cout<<"file will end soon"<<std::endl;
return;
}
in >> current;
if(current==-1)
...
{
SDL_Rect tmprect={enemies[j]->getRect()->x-baseclass::coord.x,enemies[j]->getRect()->y,50,50};
if(collision(&tmprect,player1->getRect()))
{
if(player1->getRect()->y+player1->getRect()->h>=enemies[j]->getRect()->y && player1->getRect()->y+player1->getRect()->h>=enemies[j]->getRect()->y+20)
{
delete enemies[j];
